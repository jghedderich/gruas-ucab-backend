...
namespace Orders.Application.Operators.Queries.RequestCode;
    ...
    : IQueryHandler<RequestCodeQuery, RequestCodeResult>
{
    public async Task<RequestCodeResult> Handle(RequestCodeQuery query, CancellationToken cancellationToken)
    {
        ...
        if (query.Type == "operators")
        {
            ...
            var codeDto = new RequestCodeDto(Id: operatorN.Id, Code: code, Email: operatorN.Email.Value);

            await emailSender.SendEmailAsync(operatorN.Email.Value, "Su código de recuperacion", code);
